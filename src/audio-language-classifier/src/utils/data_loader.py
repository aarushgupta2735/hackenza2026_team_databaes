...
# ── HuggingFace FLEURS streaming ──────────────────────────────────

# Mapping from our internal lang codes to FLEURS subset names
FLEURS_LANG_MAP = {
    "ar":    "ar_eg",
    "en":    "en_us",
    "fr":    "fr_fr",
...
def load_streaming_data(samples_per_lang: int = SAMPLES_PER_LANGUAGE) -> pd.DataFrame:
    """Stream audio from Google FLEURS dataset on HuggingFace.

    FLEURS (Few-shot Learning Evaluation of Universal Representations of Speech)
    is freely available and covers 102 languages.
    ...
    for lang_code, lang_name in LANGUAGES:
        ...
        if not fleurs_code:
            print(f"  ⚠ Skipping {lang_name} — no FLEURS mapping for '{lang_code}'")
            continue

        print(f"\n{'='*50}")
        print(f"  Streaming {lang_name} ({lang_code}) from FLEURS...")
        print(f"{'='*50}")
...
def split_data(df: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    ...
    if df.empty or len(df) < 10:
        raise ValueError(
            f"Not enough data to split (got {len(df)} samples). "
            f"Need at least 10 samples. Check data loading above for errors."
        )
